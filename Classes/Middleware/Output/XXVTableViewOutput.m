...
@interface XXVTableViewOutput () <UITableViewDelegate,UITableViewDataSource>
{
...
@implementation XXVTableViewOutput
...
{
    [self.refreshControl addTarget:self.interactor action:NSSelectorFromString(name) forControlEvents:UIControlEventValueChanged];
...
//- (XXVTableViewDelegate *)delegate
//{
//    if (!_delegate) {
//        _delegate = [[XXVTableViewDelegate alloc] init];
//    }
//    return _delegate;
//}
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    XXVModel* m = [self.inputer mvp_modelAtIndexPath:indexPath];
    ...
    if ([cell isKindOfClass:[XXVContentCell class]]) {
        [(XXVContentCell*)cell setPresenter:self.inputer.interactor];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.interactor respondsToSelector:@selector(mvp_action_selectItemAtIndexPath:)]) {
        [self.interactor mvp_action_selectItemAtIndexPath:indexPath];
    }
}
...
- (NSMutableArray<XXVCellActionModel *> *)actionsArrays
{
    if (!_actionsArrays) {
...
- (NSMutableArray<XXVCellActionModel *> *)leadActionsArrays
{
...
{
    NSArray* t = self.actionsArrays.filter(^BOOL(XXVCellActionModel*  _Nonnull x) {
        return x.icon == nil;
    });
    if (t.count == 0) {
        return nil;
    ...
    return
    t.map(^id _Nonnull(XXVCellActionModel*  _Nonnull x) {
        UITableViewRowAction* action = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:x.title handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
            [weakSelf.interactor mvp_runAction:x.action value:indexPath];
        }];
        if (x.color) {
