...
@property (nonatomic, weak) id<XXVModelProtocol> model;

@end

@implementation XXVContentCell
...
//    NSLog(@"%s",__func__);
    ...
    [self cleanHook];
//    id<XXVPresenterProtocol,XXVPresenterProtocol_private> presenter = [[model inputer] presenter];
//    UIViewController* vc = (UIViewController*)presenter.view;
    if ([self.model isKindOfClass:[XXVViewModel class]]) {
        XXVViewModel* m = self.model;
        UIViewController* vc2 = (UIViewController*)m.presenter;
...
//     NSLog(@"%s",__func__);
    ...
    if ([model isMemberOfClass:[XXVViewModel class]]) {
        [self loadViewModel:model];
        return;
    }
    
    id<XXVInteractorProtocol,XXVInteractorProtocol_private> presenter = self.presenter;
...
- (void)loadViewModel:(__kindof XXVViewModel*)model
{
    id<XXVInteractorProtocol,XXVInteractorProtocol_private> interactor = [[model inputer] interactor];
    UIViewController* vc = (UIViewController*)interactor.presenter;
    UIViewController* vc2 = (UIViewController*)model.presenter;
