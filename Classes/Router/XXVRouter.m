...
@implementation XXVRouter

+ (instancetype)sharedRouter
{
    static XXVRouter* sharedInstence_router = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstence_router = [[XXVRouter alloc] init];
    });
...
+ (void)registView:(Class)viewClass forURL:(NSString *)url
{
    [MGJRouter registerURLPattern:url toObjectHandler:^id(NSDictionary *routerParameters) {
        __kindof XXVPresenter* v = [[viewClass alloc] initWithUserInfo:routerParameters];
        return v;
    }];
}


+ (XXVPresenter *)viewForURL:(NSString*)url withUserInfo:(NSDictionary * _Nullable)userInfo
{
...
//    __weak typeof(self) weakSelf = self;
    ...
    [MGJRouter registerURLPattern:url toObjectHandler:^id(NSDictionary *routerParameters) {
        ...
        if (flag) {
            if ([[XXVRouter sharedRouter].cacheTable objectForKey:url]) {
                return [[XXVRouter sharedRouter].cacheTable objectForKey:url];
            }
        }
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
            ...
            [[XXVRouter sharedRouter].cacheTable setObject:[d copy] forKey:url];
            return d;
